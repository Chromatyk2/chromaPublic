            ...
            .then(function(response){
                if(response.data.filter(item => item.shiny == 0).length - 5 > -1){
                    ...
                    Axios.delete('/api/deleteShiny/'+id+"/"+pseudo)
                        .then(
                            (result) => {
                                Axios.post('/api/capture', {idUser: idUser, pseudo: pseudo, pkmName: captures[0].pkmName, pkmImage:pokemon.sprites.front_shiny,pkmId:id, shiny:1,negative:0, dateCapture:moment(new Date()).utc().format('YYYY-MM-DD hh:mm:ss')})
                                    .then(
                                        (result) => {
                                            Axios
                                                .get("/api/getByUserAndPokemon/"+pseudo+"/"+id)
